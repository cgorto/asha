...
/// Slider widget with optional [`FeathersSliderProps`].
///
/// # Emitted events
///
...
/// Properties for a [`FeathersSlider`] scene.
...
fn update_slider_pos(
    ...
    for (slider_ent, value, range, precision, mut gradient) in q_sliders.iter_mut() {
        ...
        // Update the slider's formatted text.
        q_children.iter_descendants(slider_ent).for_each(|child| {
            if let Ok(mut text) = q_slider_text.get_mut(child) {
                ...
                // Preserve values exceeding the requested precision.
...
/// Registers slider styling systems.
